...
# workflow. The only defensible reason to add one is a heredoc body that is
# *meant* to expand values which already arrived via `env:`, where quoting the
# delimiter would break it and rule 2 cannot tell that apart from the unsafe
# case.
#
# The exemption stops there. Rule 1 still runs on an exempted file: it is the
# primary injection class, has nothing to do with heredoc quoting, and an
# already-expanding body is the worst place to leave unchecked — a substituted
# `$(...)` would execute rather than merely mis-parse.
#
# Empty is the intended state. No workflow here has argued its way in yet.
HEREDOC_EXCEPTIONS=()

# The reusable set, derived from the `workflow_call` trigger. Classification
# lives in scripts/list-reusable-workflows.sh rather than inline, so a second
# consumer of the same set cannot drift away from what this guard scans.
# Command substitution, not a process substitution: `done < <(...)` would not
# propagate the classifier's exit status, so a parse error would yield a
# silently shortened set and a green run.
